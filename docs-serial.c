...
int num_arm = 0;
  ...
  // Cria��o dos documentos e cria��o de asuntos
  ...
  for (i = 0; i < nDocumentos; i++) {
    ...
    // Adiciona o documento no arm�rio correspondente
    int resto = documentos[i].idD % nArmarios;   //pega o resto da divis�o
    Ar *armario = &armarios[resto];            //cria ponteiro que aponta para o endere�o armario em que vamos guardar o documento
    documentos[i].prox = armario->listDoc;
	documentos[i].armarioPertence=resto;    //novo documento aponta para o primeiro documento daquele arm�rio
    if (armario->listDoc != NULL) {
   armario->listDoc->ant = &documentos[i];
	}
    armario->listDoc = &documentos[i];      //ponteiro daquele arm�rio aponta para novo documento
...
do{
control=0;
 //Organiza��o dos documentos nos arm�rios com base na dist�ncia
  for(i = 0; i < nDocumentos; i++) {
...
// Adiciona o documento no arm�rio com a menor dist�ncia
	Dc *doc = &documentos[i];
	Ar *armario = &armarios[indiceMenorDistancia];
		if(doc->armarioPertence!=armario->idA)
		control=1;
    
		// Remove o documento da lista do arm�rio anterior, se houver
		armarios[doc->armarioPertence].numDocs--; //diminuir o peso do armario aonde o documento sair�	
		//SE ESTIVER NO MEIO
		...
		// Insere o documento na lista do arm�rio selecionado
		if (armario->listDoc != NULL) {
 ...
 // Impress�o dos arm�rios e seus documentos
...
///////////////////////////////////////////////////////////////////////////////////////////////
 ...
 //calcular os pesos dos arm�rios
 void calculaPeso(int nArmarios,int nAssuntos, Ar *armarios){
	 int indice;
 double soma=0;
 int k;
